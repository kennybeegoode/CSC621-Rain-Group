...
void RegionGrowingNoThreshold::ComputeStats(InternalImageType::Pointer& image,
  ...
  *mgv = (double) sum / region_points.size();
...
void RegionGrowingNoThreshold::ComputeThreshold(
  ...
  *upper_threshold = mgv + (upper_dev * 1.5 + 20.0 / sqrt(region_points.size()));
  *lower_threshold = mgv - (lower_dev * 1.5 + 20.0 / sqrt(region_points.size()));
...
void RegionGrowingNoThreshold::ComputeFinalThreshold(
  ...
  *upper_threshold = mgv + (*upper_dev * 2.3 * 2.58 + 20.0 / sqrt(region_points.size()));
  *lower_threshold = mgv - (*lower_dev * 2.3 * 2.58 + 20.0 / sqrt(region_points.size()));
